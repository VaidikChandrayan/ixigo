Feature: Ixigo Flight Search Module
  ...
  @filter
  Scenario: User applies airline filter
    When User enters source and destination with date and clicks on search
    And User clicks on airline filter

  @partialtext
  Scenario Outline: Validate partial text in From and To fields should show suggestions
    When the user enters partial city name in From field and To field
    Then the From field should contain city name
    And the To field should contain city name
