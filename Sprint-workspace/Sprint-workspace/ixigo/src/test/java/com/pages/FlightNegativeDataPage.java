...
public class FlightNegativeDataPage extends BasePage {
	...
	public void ToSameCity() throws AWTException, InterruptedException {
		...
		WebElement toInput = wait.until(
				ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(text(),'To')]/following::input[1]")));
		toInput.click();
	...
	public void clickAirIndiaFilter() {
		...
		WebElement airIndiaCheckbox = wait.until(ExpectedConditions
				.elementToBeClickable(By.xpath("//label[contains(., 'Air India')]//input[@type='checkbox']")));

		// Scroll into view
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block:'center'});", airIndiaCheckbox);
