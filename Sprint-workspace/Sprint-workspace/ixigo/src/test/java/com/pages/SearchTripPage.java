...
public class SearchTripPage extends BasePage {
	...
	public void switchToFlightResultTab() {
		String originalTab = driver.getWindowHandle();

		// Wait for new tab to appear
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
		wait.until(driver -> driver.getWindowHandles().size() > 1); // Wait until a new browser tab is opened (i.e.,
																	// when total tabs > 1) // This ensures that we only
																	// switch tabs after the flight result tab has
																	// successfully loaded
		...
		for (String handle : allTabs) {
			if (!handle.equals(originalTab)) {
				...
				System.out.println("Switched to flight results tab: " + driver.getCurrentUrl());
				break;
			}
		}
	}
