...
public class FilterDataPage extends BasePage {
	...
	public void filter() throws InterruptedException {
		...
		try {
			// Wait for Air India filter text
			...
			// Dismiss discount popup by clicking on body
			try {
				((JavascriptExecutor) driver).executeScript("document.body.click();");
				System.out.println("Popup dismissed by clicking on the page");
				Thread.sleep(1000); // Let UI settle
			} catch (Exception e) {
				System.out.println("Failed to dismiss popup: " + e.getMessage());
			}

			// Scroll to Air India element
