...
public class BasePage {
	...
	private void loadProperties() 
	{
        try {
            prop = new Properties();
            FileInputStream fis = new FileInputStream("C:\\Sprint-workspace\\ixigo\\src\\test\\resource\\PropertieFiles\\FlightSearch.properties");
            prop.load(fis);
        } catch (Exception e) {
            e.printStackTrace();
        }
	...
	public void ToData()
        {
		  WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(15));
		    ...
		    ((JavascriptExecutor) driver).executeScript("arguments[0].value='DEL';", toInput);

		    // Wait for the suggestion to appear
		    WebElement suggestion = wait.until(ExpectedConditions.elementToBeClickable(delhiOption));

		    // Click the suggestion using JavaScript
